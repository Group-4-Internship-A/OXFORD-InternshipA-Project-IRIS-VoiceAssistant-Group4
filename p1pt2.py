...
def core():
    ...
    while 'quit' not in user_response:
        if 'time' in user_response:
            ...
            speak('Alrighty! Who would you like to send an email to,' + nme)
            recip = rec_user()
            speak('Got it. Tell me the content of the mail.')
            cont = rec_user()
            speak('Cool, hang on' + nme)
            sendEmail(directory[recip], cont)
            speak('Sent')
            return
        elif 'send a whatsapp message' in user_response.lower() or 'send whatsapp message' in user_response.lower() or 'send a message' in user_response.lower() or 'send message' in user_response.lower() or 'whatsapp' in user_response.lower():
            wa()
            return
